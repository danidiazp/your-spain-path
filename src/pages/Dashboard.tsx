...
import { CheckCircle2, Circle, ArrowRight, FileText, Calendar, MapPin, BookOpen, Sparkles, AlertCircle, ListChecks } from "lucide-react";
...
type Route = { id: string; slug: string; name: string; short_description: string | null; estimated_timeline: string | null };
type Step = { id: string; step_order: number; title: string; description: string | null; estimated_duration: string | null; stage_location: string | null };
type Doc = { id: string; name: string; description: string | null; required: boolean | null; translation_needed: boolean | null; apostille_needed: boolean | null };
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (assess?.primary_route_id) {
        const [{ data: r }, { data: s }, { data: d }] = await Promise.all([
          supabase.from("migration_routes").select("id, slug, name, short_description, estimated_timeline").eq("id", assess.primary_route_id).maybeSingle(),
          supabase.from("route_steps").select("id, step_order, title, description, estimated_duration, stage_location").eq("route_id", assess.primary_route_id).order("step_order"),
          supabase.from("route_documents").select("id, name, description, required, translation_needed, apostille_needed").eq("route_id", assess.primary_route_id),
        ]);
        setRoute(r);
        setSteps(s ?? []);
        setDocs(d ?? []);
      ...
      const { data: t } = await supabase.from("user_tasks").select("id, title, status, description").eq("profile_id", user.id);
      setTasks(t ?? []);
      setLoading(false);
    })();
  }, [user]);
  ...
  const seedTasksFromRoute = async () => {
    ...
    else { setTasks(data ?? []); toast.success("Tareas creadas a partir de tu ruta"); }
  };
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="lg:col-span-2 bg-gradient-primary rounded-3xl p-7 lg:p-8 text-primary-foreground relative overflow-hidden shadow-deep">
            ...
            <div className="relative space-y-4">
              <p className="text-xs uppercase tracking-[0.18em] opacity-75">Ruta principal</p>
              <h2 className="font-display text-3xl lg:text-4xl font-semibold">{route?.name ?? "Ruta no encontrada"}</h2>
              <p className="opacity-85 max-w-xl">{route?.short_description}</p>
        ...
        {nextTask && (
          <div className="bg-accent-soft/50 border border-accent/30 rounded-3xl p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs uppercase tracking-wider text-muted-foreground mb-0.5">Próximo paso</p>
                <p className="font-medium">{nextTask.title}</p>
              </div>
            </div>
            <Button variant="accent" size="sm" onClick={() => toggleTask(nextTask.id, nextTask.status)}>
                ...
                {tasks.map((t) => (
                  <li key={t.id}>
                    <button
                      ...
                      className="w-full flex items-start gap-3 p-2.5 rounded-xl hover:bg-secondary/60 text-left transition-colors"
                    >
                      {t.status === "done" ? (
          ...
          <ol className="relative border-l-2 border-border ml-4 space-y-6">
            {steps.map((s) => (
              <li key={s.id} className="ml-6">
                <span className="absolute -left-[11px] flex h-5 w-5 items-center justify-center rounded-full bg-primary text-primary-foreground text-[10px] font-semibold">{s.step_order}</span>
        ...
        {assessment.missing_requirements_json && Array.isArray(assessment.missing_requirements_json) && assessment.missing_requirements_json.length > 0 && (
          <div className="bg-warning/10 border border-warning/30 rounded-3xl p-6 lg:p-7">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-display text-lg font-semibold">Alertas y requisitos pendientes</h3>
            </div>
            <ul className="space-y-2 ml-7 list-disc text-sm text-muted-foreground">
        ...
        <div className="bg-card border border-border rounded-3xl p-6 lg:p-7 shadow-elegant">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display text-lg font-semibold">Recursos oficiales</h3>
            </div>
            <Button asChild variant="ghost" size="sm">
              <Link to="/recursos">Ver biblioteca <ArrowRight className="h-3.5 w-3.5" /></Link>
            </Button>
          </div>
          <p className="text-sm text-muted-foreground">Accede a fuentes verificadas del Gobierno de España, citas, sedes electrónicas y consulados.</p>
        </div>
